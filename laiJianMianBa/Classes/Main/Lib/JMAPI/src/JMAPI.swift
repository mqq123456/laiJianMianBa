...
// MARK: - 业务相关
    ...
    ///
    /// - Parameter request: 请求参数
    open func jmGetByToUserIdWithPage(_ request: RequestGetByToUserIdWithPageModel) {
        let parse = request.parseData()
        print("request = \(JSON(parse))")
        NetworkRequest.sharedInstance.postRequest(urlString: url, params: parse, success: { (resp) in
            let response = ResponseGetByToUserIdWithPageModel.parseJson(resp as NSDictionary)
            self.delegate?.jmGetByToUserIdWithPageDone!(request: request, response: response)
        }) { (error) in
            print("error = \(error)")
            self.delegate?.jmRequestFail!(request, error: error as NSError)
        }
    ...
    ///
    /// - Parameter request: 请求参数
    open func jmGetImpress(_ request: RequestGetImpressModel) {
        let parse = request.parseData()
        print("request = \(JSON(parse))")
        NetworkRequest.sharedInstance.postRequest(urlString: url, params: parse, success: { (resp) in
            let response = ResponseGetImpressModel.parseJson(resp as NSDictionary)
            self.delegate?.jmGetImpressDone!(request: request, response: response)
        }) { (error) in
            print("error = \(error)")
            self.delegate?.jmRequestFail!(request, error: error as NSError)
        }
    ...
    ///
    /// - Parameter request: 请求参数
    open func jmSetPhotoMatch(_ request: RequestSetPhotoMatchModel) {
        let parse = request.parseData()
        print("request = \(JSON(parse))")
        NetworkRequest.sharedInstance.postRequest(urlString: url, params: parse, success: { (resp) in
            let response = ResponseSetPhotoMatchModel.parseJson(resp as NSDictionary)
            self.delegate?.jmSetPhotoMatchDone!(request: request, response: response)
        }) { (error) in
            print("error = \(error)")
            self.delegate?.jmRequestFail!(request, error: error as NSError)
        }
    ...
    ///
    /// - Parameter request: 请求参数
    open func jmUpdateOrderStateMeeted(_ request: RequestUpdateOrderStateMeetedModel) {
        let parse = request.parseData()
        print("request = \(JSON(parse))")
        NetworkRequest.sharedInstance.postRequest(urlString: url, params: parse, success: { (resp) in
            let response = ResponseUpdateOrderStateMeetedModel.parseJson(resp as NSDictionary)
            self.delegate?.jmUpdateOrderStateMeetedDone!(request: request, response: response)
        }) { (error) in
            print("error = \(error)")
            self.delegate?.jmRequestFail!(request, error: error as NSError)
        }
